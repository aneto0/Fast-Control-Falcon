...
bool FilterDownsamplingGAM::Setup() {
    ...
    if (ok) {
        ...
        for (i = 0u; (i < numberOfSignals) && (ok); i++) {
            ...
            if (!ok) {
                REPORT_ERROR(ErrorManagement::ParametersError, "GetSignalType(InputSignals) != Float32Bit");
            }
        }
    }
    if (ok) {
    ...
    if (ok) {
        ...
        for (s = 0u; (s < numberOfSignals) && (ok); s++) {
            ...
            output[s] = static_cast<float32 *>(GetOutputSignalMemory(s));
            accumulator[s] = 0;
        }
    }
    if (ok) {
...
bool FilterDownsamplingGAM::Execute() {
    ...
    for (s = 0u; s < numberOfSignals; s++) {
        ...
        *(output[s]) = static_cast<float32>(accumulator[s] * gain[s]);
    }
    return true;
}
